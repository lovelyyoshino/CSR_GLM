...
def __is_any_api_key(key:str):
    if ',' in key:
        keys = key.split(',')
        for k in keys:
            if __is_any_api_key(k): return True
        return False
    else:
...
def __read_env_variable(arg, default_value):
    ...
    if arg_with_prefix in os.environ: # 优先使用带前缀的环境变量
        ...
        raise KeyError
    print(f"[ENV_VAR] 尝试加载{arg}，默认值：{default_value} --> 修正值：{env_arg}")# 修正值是指环境变量的值,只有存在才会打印，否则会raise KeyError
    try:
...
def __read_single_conf_with_lru_cache(arg):
    ...
    try:
        # 优先级1. 获取环境变量作为配置
        default_ref = getattr(importlib.import_module('config'), arg)   # 读取默认值作为数据类型转换的参考，importlib.import_module('config')是动态地获取另一个py文件中定义，然后getattr()获取其中arg的变量
...
def select_api_key(keys:str , llm_model:str):
    ...
    if llm_model.startswith('gpt-'):
        for k in key_list:
            if __is_openai_api_key(k): avail_key_list.append(k)

    if llm_model.startswith('api2d-'):
        for k in key_list:
            if __is_api2d_key(k): avail_key_list.append(k)
