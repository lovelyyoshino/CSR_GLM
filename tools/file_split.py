...
"""
def breakdown_txt_to_satisfy_token_limit(txt:str, get_token_fn, limit:int, must_break_at_empty_line=True):
    if get_token_fn(txt) <= limit: # 如果长度小于限制，则直接返回
        ...
        if cnt == 0:# 如果切割到第一行，则报错，并直接返回
            if must_break_at_empty_line == False:
                print亮蓝("无法再切割")
                return []
            breakdown_txt_to_satisfy_token_limit(txt,get_token_fn, limit, must_break_at_empty_line=False)#直到没办法再切割为止，然后就返回
